...
export const StudentExam: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-slate-900 flex flex-col font-sans">
      {/* Top Header Bar */}
      <header className="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 px-4 py-3 sticky top-0 z-30 flex items-center justify-between">
        <div className="flex items-center space-x-3">
          <div className="text-sm font-extrabold text-slate-800 dark:text-slate-100">{quiz.title}</div>
          <span className="text-xs text-slate-400 hidden sm:inline">• {participant.fullName} ({participant.studentClass})</span>
        </div>

        {/* Big Timer */}
        <div className="flex items-center space-x-3">
          <div className={`px-4 py-1.5 rounded-xl flex items-center space-x-2 font-mono font-black text-base shadow-sm ${
